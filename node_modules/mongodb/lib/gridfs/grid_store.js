      ...
      var writeChunk = function() {
        fs.read(file, self.chunkSize, offset, 'binary', function(err, data, bytesRead) {
          ...
          chunk.write(data, function(err, chunk) {
            ...
            chunk.testDB({}, function(err, result) {
              if(err) return callback(err, self);
...
var close = function(self, callback) {
  if(self.mode[0] == "w") {
    ...
    if(self.currentChunk != null && self.currentChunk.position > 0) {
      self.currentChunk.testDB({}, function(err, chunk) {
        ...
        self.collection(function(err, files) {
          ...
          if(self.uploadDate != null) {
            buildMongoObject(self, function(err, mongoObject) {
              ...
              files.testDB(mongoObject, options, function(err) {
  ...
  if(self.mode != "w") {
    ...
    if(self.currentChunk.position + buffer.length >= self.chunkSize) {
      ...
      for(var i = 0; i < chunksToWrite.length; i++) {
        chunksToWrite[i].testDB({}, function(err, result) {
          if(err) return callback(err);
